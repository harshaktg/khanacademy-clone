...
<template>
    <div class="flex flex-col relative items-stretch border-b border-[#21242c52] lg:grid lg:grid-cols-[1fr_1fr]"
        :class="course.class">
        <button @click="toggleExpand" :aria-expanded="isExpanded" type="button"
            ...
            <img :src="course.icon" width="45" height="45"
                class="rounded-full mr-3 w-[34px] h-[34px] lg:w-[45px] lg:h-[45px]" :alt="course.name">
            {{ course.name }}
            <span class="caret-down" :class="{ 'collapsed': !isExpanded }"></span>
        </button>
